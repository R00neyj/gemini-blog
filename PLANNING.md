...
8. **UI/UX 개선:** 
   ...
   - **반응형 네비게이션 구조 개편:**
     ...
     - **Mobile:** 최신 Apple UI 스타일의 하단 플로팅 네비게이션 바(BottomNav).
       ...
       - **UI 디테일 개선:**
         - PC 사이드바: 메뉴 버튼의 그라데이션 제거, 심플한 투명/호버 효과로 변경.
         - 설정 페이지: 컨텐츠 섹션의 회색 배경 박스 제거, 투명한 배경으로 통합.
         - 모바일 네비바: 높이 축소, 선택된 아이템의 라운드 값 증가.
         - 모바일 FAB: 네비바 높이와 시각적 균형을 맞추도록 크기 조정 (원형 유지).
         - PWA 설치 팝업: 배경 클릭 시 닫힘 처리.
